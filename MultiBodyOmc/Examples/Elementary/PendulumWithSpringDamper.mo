...
model PendulumWithSpringDamper "Simple spring/damper/mass system"
  ...
  inner MultiBodyOmc.World world(axisLength=0.6)
  ...
  MultiBodyOmc.Forces.Spring spring1(
    ...
    s_unstretched=0.2) annotation (Placement(transformation(extent={{0,-46},{20,
            -26}}, rotation=0)));
  MultiBodyOmc.Forces.Damper damper1(
    d=1,
    length_a=0.1,
    diameter_a=0.08,
    animation=false) annotation (Placement(transformation(extent={{0,-20},{20,0}},
          rotation=0)));
  MultiBodyOmc.Joints.Revolute revolute(phi(fixed=true), w(
        fixed=true))      annotation (Placement(transformation(extent={{-12,20},
            {8,40}}, rotation=0)));
  MultiBodyOmc.Joints.Prismatic prismatic(
    boxWidth=0.04,
    boxColor={255,65,65},
