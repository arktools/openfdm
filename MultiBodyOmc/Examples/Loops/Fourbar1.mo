...
model Fourbar1
  ...
  MultiBodyOmc.Joints.Revolute j1(
    n={1,0,0},
    ...
    w(displayUnit="deg/s",
      ...
      fixed=true))
            ...
            -20}}, rotation=0)));
  MultiBodyOmc.Joints.Prismatic j2(
    n={1,0,0},
    s(start=-0.2),
    boxWidth=0.05) annotation (Placement(transformation(extent={{10,-80},{30,
            -60}}, rotation=0)));
  MultiBodyOmc.Parts.BodyCylinder b1(r={0,0.5,0.1}, diameter=0.05)
    annotation (Placement(transformation(
        origin={-30,2},
        extent={{-10,-10},{10,10}},
        rotation=90)));
  MultiBodyOmc.Parts.BodyCylinder b2(r={0,0.2,0}, diameter=0.05)
    annotation (Placement(transformation(
        origin={50,-50},
        extent={{-10,-10},{10,10}},
        rotation=90)));
  MultiBodyOmc.Parts.BodyCylinder b3(r={-1,0.3,0.1}, diameter=0.05)
    annotation (Placement(transformation(extent={{38,20},{18,40}}, rotation=0)));
  MultiBodyOmc.Joints.Revolute rev(n={0,1,0})
    annotation (Placement(transformation(
        origin={50,-22},
        extent={{-10,-10},{10,10}},
        rotation=90)));
  MultiBodyOmc.Joints.Revolute rev1 annotation (Placement(
        transformation(extent={{60,0},{80,20}}, rotation=0)));
  MultiBodyOmc.Joints.Revolute j3(n={1,0,0}) annotation (Placement(
        transformation(extent={{-60,40},{-40,60}}, rotation=0)));
  MultiBodyOmc.Joints.Revolute j4(n={0,1,0}) annotation (Placement(
        transformation(extent={{-32,60},{-12,80}}, rotation=0)));
  MultiBodyOmc.Joints.Revolute j5(n={0,0,1}) annotation (Placement(
        transformation(extent={{0,70},{20,90}}, rotation=0)));
  MultiBodyOmc.Parts.FixedTranslation b0(animation=false, r={1.2,0,0})
