...
/*
  ...
  CombiTable1D czpTable(columns={2}, table = transpose(
   {{   -10,     -5,      0,      5,     10,     15,     20,     25,     30,     35,     40,     45},
    { -8.80,  -25.8,  -28.9,  -31.4,  -31.2,  -30.7,  -27.7,  -28.2,  -29.0,  -29.8,  -38.3,  -35.3}}))
    "(what does table do??) table(aoa[deg])";

  CombiTable1D clrTable(columns={2}, table = transpose(
   {{   -10,     -5,      0,      5,     10,     15,     20,     25,     30,     35,     40,     45}, 
  ...
  CombiTable1D clpTable(columns={2}, table = transpose(
   {{    -10,     -5,      0,      5,     10,     15,     20,     25,     30,     35,     40,     45},
  ...
  CombiTable1D cnpTable(columns={2}, table = transpose(
   {{    -10,     -5,      0,      5,     10,     15,     20,     25,     30,     35,     40,     45},
...
*/
  ...
  CombiTable1D czpTable(table=test1D);
  CombiTable1D clrTable(table=test1D);
  CombiTable1D clpTable(table=test1D);
  CombiTable1D cmqTable(table=test1D);
  CombiTable1D cnrTable(table=test1D);
  CombiTable1D cnpTable(table=test1D);
  CombiTable1D czTable(table=test1D);
  CombiTable2D cxTable(table=test2D);
  CombiTable2D cmTable(table=test2D);
  CombiTable2D clTable(table=test2D);
  CombiTable2D cnTable(table=test2D);
  CombiTable2D dldrTable(table=test2D);
  ...
  Real cx0, cz0, cl0, cm0, cn0, dlda, dldr, dnda, dndr;
protected
  // local var for table use
  Real tvt, b2v, cq, daileron, drudder, dbeta, delevator; 

algorithm

  daileron := aileron_deg/20.0;
  ...
  if (vt<vtTol) then
    ...
    cy := -0.02*beta_deg +
      0.021*daileron +
      0.086 * drudder +
      b2v* ( cyr*aero_r + cyp*aero_p );
    cz := cz0*(1-dbeta^2)-0.19*delevator + cq * czp;
    cl := cl0*sign(beta_deg) +
      dlda*daileron + dldr*drudder +
      b2v * ( clr*aero_r + clp*aero_p );
...
equation
  ...
  connect(cxTable.u1, elevator_deg);
  connect(cxTable.u2, alpha_deg);
  connect(cmTable.u1, beta_deg_abs);
  connect(cmTable.u2, alpha_deg);
  connect(clTable.u1, beta_deg_abs);
  connect(clTable.u2, alpha_deg);
  connect(cnTable.u1, beta_deg_abs);
  connect(cnTable.u2, alpha_deg);
  connect(dldaTable.u1, beta_deg_abs);
  connect(dldaTable.u2, alpha_deg);
  connect(dldrTable.u1, beta_deg_abs);
  connect(dldrTable.u2, alpha_deg);
  connect(dndaTable.u1, beta_deg_abs);
  connect(dndaTable.u2, alpha_deg);
  connect(dndrTable.u1, beta_deg_abs);
