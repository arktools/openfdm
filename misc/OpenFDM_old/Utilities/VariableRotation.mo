...
model VariableRotation
  ...
  MB.Frames.Orientation R_rel "relative rotation from frame_a to frame_b";
equation
  ...
  R_rel = MB.Frames.axisRotation(2,angle,angleDot);

  /* Relationships between quantities of frame_a and frame_b */
  frame_b.r_0 = frame_a.r_0;
  frame_b.R = MB.Frames.absoluteRotation(frame_a.R, R_rel);
  zeros(3) = frame_a.f + MB.Frames.resolve1(R_rel, frame_b.f);
  zeros(3) = frame_a.t + MB.Frames.resolve1(R_rel, frame_b.t);
