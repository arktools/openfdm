...
model RollingWheelSetPulling "Rolling wheel set that is pulled by a force"
  ...
  MultiBodyOmc.Forces.WorldForce force(animation=false)
    annotation (Placement(transformation(extent={{-20,60},{0,80}})));
  MultiBodyOmc.Visualizers.Ground ground(
                length=3)
    annotation (Placement(transformation(extent={{-20,-40},{0,-20}})));
  inner MultiBodyOmc.World world(label2="z", n={0,0,-1})
    annotation (Placement(transformation(extent={{-60,0},{-40,20}})));
  MultiBodyOmc.Parts.RollingWheelSet wheelSet(
    wheelRadius=0.1,
    wheelMass=0.5,
    wheel_I_axis=0.01,
    wheel_I_long=0.02,
  ...
  MultiBodyOmc.Parts.Body body(m=0.01, r_CM={0,0,0},
    animation=false)
  ...
  MultiBodyOmc.Parts.FixedTranslation fixedTranslation(
                       r={0.2,0,0},
    animation=true,
    width=0.04)
    annotation (Placement(transformation(extent={{0,20},{20,40}})));
  MultiBodyOmc.Visualizers.FixedShape shape(
