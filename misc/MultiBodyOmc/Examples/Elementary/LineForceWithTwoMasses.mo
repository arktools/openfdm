within MultiBodyOmc.Examples.Elementary;
model LineForceWithTwoMasses
  ...
  MultiBodyOmc.Joints.Revolute revolute1(phi(fixed=true), w(
        fixed=true))      annotation (Placement(transformation(extent={{-20,60},
            {0,80}}, rotation=0)));
  MultiBodyOmc.Parts.BodyBox bodyBox1(r={0.7,0,0})
    annotation (Placement(transformation(extent={{20,60},{40,80}}, rotation=0)));
  MultiBodyOmc.Parts.FixedTranslation rod1(
    r={0,-0.9,0},
    width=0.01,
    animation=false) annotation (Placement(transformation(
        ...
        extent={{-10,-10},{10,10}},
        rotation=270)));
  MultiBodyOmc.Joints.Assemblies.JointUPS jointUPS(nAxis_ia={0.7,1.2,0}, animation=
  ...
  MultiBodyOmc.Joints.Revolute revolute2(phi(fixed=true), w(
        fixed=true))      annotation (Placement(transformation(extent={{-20,-40},
            {0,-20}}, rotation=0)));
  MultiBodyOmc.Parts.BodyBox bodyBox2(r={0.7,0,0})
    annotation (Placement(transformation(extent={{20,-40},{40,-20}}, rotation=0)));
  MultiBodyOmc.Parts.FixedTranslation rod3(
    width=0.01,
    r={0,-0.9,0.3},
    animation=false) annotation (Placement(transformation(
        origin={-40,-46},
        extent={{-10,-10},{10,10}},
        rotation=270)));
  MultiBodyOmc.Parts.FixedTranslation rod4(
    width=0.01,
    r={0,0.3,0.3},
    animation=false) annotation (Placement(transformation(
        origin={-40,-16},
  ...
  MultiBodyOmc.Forces.LineForceWithTwoMasses
...
<ul>
<li> With
     <a href=\"Modelica://MultiBodyOmc.Joints.Assemblies.JointUPS\">JointUPS</a>:<br>
     MultiBodyOmc.Joints.Assemblies.JointUPS is an aggregation
...
<li> With <a href=\"Modelica://MultiBodyOmc.Forces.LineForceWithTwoMasses\">
     LineForceWithTwoMasses</a>:<br>
     MultiBodyOmc.Forces.LineForceWithTwoMasses is a line force component
